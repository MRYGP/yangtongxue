...
# 检查Fairness三句式（应该有2次）
fairness_count = len(re.findall(r"Adjust task format without lowering challenge intensity; assess by relative improvement rather than absolute level; \\(if involving accessibility\\) challenge budget conservation\\.", t))
...
# 检查P2公式
has_p2_formula = bool(re.search(r"\\$P_2.*\\\\ge.*B_0.*\\\\+.*\\\\delta\\$", t))
